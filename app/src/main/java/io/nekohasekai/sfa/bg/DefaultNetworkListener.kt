...
object DefaultNetworkListener {
    ...
    suspend fun get(): Network = if (fallback) {
        @TargetApi(23)
        PluginManager.connectivity.activeNetwork
    ...
    private fun register() {
        when (Build.VERSION.SDK_INT) {
            in 31..Int.MAX_VALUE ->
                @TargetApi(31)
                {
                    PluginManager.connectivity.registerBestMatchingNetworkCallback(
                        request,
                        Callback,
                        mainHandler,
                    )
            ...
            in 28 until 31 ->
                ...
                { // we want REQUEST here instead of LISTEN
                    PluginManager.connectivity.requestNetwork(request, Callback, mainHandler)
                }

            in 26 until 28 ->
                @TargetApi(26)
                {
                    PluginManager.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
                }

            in 24 until 26 ->
                @TargetApi(24)
                {
                    PluginManager.connectivity.registerDefaultNetworkCallback(Callback)
                }

            else ->
                try {
                    fallback = false
                    PluginManager.connectivity.requestNetwork(request, Callback)
    ...
    private fun unregister() {
        runCatching {
            PluginManager.connectivity.unregisterNetworkCallback(Callback)
        }
    }
}
